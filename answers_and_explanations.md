...
#### 문제 1
**문제**: 서비스 워커가 HTTPS 환경에서만 작동하는 이유로 가장 적절한 것은?

**정답**: 중간자 공격과 같은 보안 위협으로부터 보호하기 위해

**해설**: 서비스 워커는 네트워크 요청을 가로채고 수정할 수 있는 강력한 기능을 가지고 있기 때문에, 보안상의 이유로 HTTPS 환경에서만 작동하도록 설계되었습니다. HTTPS는 통신 내용이 제3자에 의해 가로채지거나 변조되는 중간자 공격을 방지하고, 웹사이트의 신원을 확인할 수 있게 합니다. 이를 통해 서비스 워커의 강력한 기능이 악의적인 목적으로 사용되는 것을 방지하고, 사용자의 데이터와 프라이버시를 보호할 수 있습니다. 서비스 워커의 성능 최적화, 브라우저 호환성 문제 해결, 설치 속도 향상은 HTTPS 요구사항의 주된 이유가 아닙니다.

#### 문제 2
**문제**: 서비스 워커의 스코프에 대한 설명으로 올바르지 않은 것은?

**정답**: 서비스 워커는 기본적으로 웹사이트의 모든 경로에 영향을 미친다

**해설**: 서비스 워커는 기본적으로 웹사이트의 모든 경로에 영향을 미치는 것이 아니라, 자신이 위치한 디렉토리와 그 하위 디렉토리에만 영향을 미칩니다. 이는 보안상의 이유로 서비스 워커의 영향 범위를 제한하기 위한 것입니다. 예를 들어, `/app/sw.js`에 위치한 서비스 워커는 기본적으로 `/app/` 경로와 그 하위 경로에만 영향을 미치며, `/` 경로에는 영향을 미치지 않습니다. 서비스 워커의 스코프는 등록 시 옵션으로 명시적으로 지정할 수 있으며, Service-Worker-Allowed 헤더를 사용하면 서비스 워커의 스코프를 확장할 수 있습니다. 이러한 스코프 제한은 권한 분리와 공격 표면 감소에 기여합니다.

#### 문제 3
**문제**: 서비스 워커에서 민감한 데이터를 처리할 때 권장되는 방법은? (복수 응답)

**정답**: 민감한 데이터를 포함하는 응답은 캐싱하지 않는다, Cache-Control 헤더를 활용하여 캐싱 동작을 제어한다, 필요한 경우 데이터를 암호화하여 저장한다

**해설**: 
- 민감한 데이터를 포함하는 응답은 캐싱하지 않는 것이 권장됩니다. 인증 정보, 개인 식별 정보, 금융 정보 등은 캐시에 저장되면 보안 위험이 발생할 수 있습니다.
- Cache-Control 헤더를 활용하여 캐싱 동작을 제어하는 것도 권장됩니다. 서버에서 'no-store' 지시문을 포함한 Cache-Control 헤더를 설정하여 민감한 데이터가 캐시되지 않도록 할 수 있습니다.
- 필요한 경우 데이터를 암호화하여 저장하는 것도 권장됩니다. 오프라인 기능을 위해 민감한 데이터를 저장해야 하는 경우, Web Crypto API 등을 사용하여 데이터를 암호화하여 저장해야 합니다.
- 모든 API 응답을 무조건 캐싱하는 것은 권장되지 않습니다. 이는 민감한 데이터가 캐시에 저장되어 보안 위험을 초래할 수 있습니다.
- 민감한 데이터를 영구적으로 저장하는 것도 권장되지 않습니다. 민감한 데이터는 필요한 기간 동안만 저장하고, 더 이상 필요하지 않을 때 안전하게 삭제해야 합니다.

#### 문제 4
**문제**: 서비스 워커 보안을 강화하기 위한 모범 사례로 올바른 것은? (복수 응답)

**정답**: 서비스 워커 스크립트의 무결성을 확인한다, 정기적으로 서비스 워커를 업데이트하고 버전을 관리한다, Content-Security-Policy(CSP)를 설정하여 리소스 로드를 제한한다, 서비스 워커의 기능과 데이터 사용에 대해 사용자에게 투명하게 알린다

**해설**: 
- 서비스 워커 스크립트의 무결성을 확인하는 것은 악의적인 코드 삽입을 방지하기 위한 중요한 보안 모범 사례입니다. 서브리소스 무결성(SRI)을 적용하거나 해시 검증을 통해 스크립트의 무결성을 보장할 수 있습니다.
- 정기적으로 서비스 워커를 업데이트하고 버전을 관리하는 것도 중요한 모범 사례입니다. 보안 취약점이 발견되면 신속하게 패치하고, 적절한 버전 관리 전략을 통해 업데이트를 효과적으로 배포해야 합니다.
- Content-Security-Policy(CSP)를 설정하여 리소스 로드를 제한하는 것도 중요한 모범 사례입니다. CSP는 허용된 소스에서만 리소스를 로드하도록 제한하여 XSS 공격 등을 방지할 수 있습니다.
- 서비스 워커의 기능과 데이터 사용에 대해 사용자에게 투명하게 알리는 것도 중요한 모범 사례입니다. 사용자의 동의를 얻고, 데이터 사용 방식을 명확하게 설명하여 신뢰를 구축해야 합니다.
- 모든 사용자 데이터를 평문으로 저장하는 것은 보안 모범 사례가 아닙니다. 민감한 데이터는 암호화하여 저장해야 합니다.

#### 문제 5
**문제**: 다음 중 서비스 워커 스코프 제한의 보안 이점이 아닌 것은?

**정답**: 서비스 워커의 실행 속도 향상

**해설**: 서비스 워커 스코프 제한의 보안 이점에는 권한 분리 원칙 적용, 잠재적인 공격 표면 감소, 기능별로 다른 서비스 워커 사용 가능 등이 있습니다. 그러나 서비스 워커의 실행 속도 향상은 스코프 제한의 보안 이점이 아닙니다. 스코프 제한은 서비스 워커의 영향 범위를 제한하여 보안을 강화하는 것이 주목적이며, 실행 속도와는 직접적인 관련이 없습니다. 권한 분리 원칙을 적용하면 각 서비스 워커가 자신의 영역만 제어하도록 제한하여 한 영역의 취약점이 다른 영역에 영향을 미치지 않도록 할 수 있습니다. 또한 서비스 워커의 영향 범위를 제한하여 잠재적인 공격 표면을 줄이고, 웹사이트의 다른 부분에 대해 다른 서비스 워커를 사용하여 기능별 분리가 가능합니다.
