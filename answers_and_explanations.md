...
#### 문제 1
**문제**: Workbox의 주요 목적은 무엇인가요?

**정답**: 서비스 워커 개발 단순화 및 최적화

**해설**: Workbox의 주요 목적은 서비스 워커 개발을 단순화하고 최적화하는 것입니다. Workbox는 Google에서 개발한 라이브러리 모음으로, 서비스 워커의 복잡성을 추상화하여 개발자가 더 쉽게 PWA 기능을 구현할 수 있게 해줍니다. 모듈식 구조, 다양한 캐싱 전략, 자동화된 서비스 워커 생성, 빌드 도구 통합 등의 기능을 제공하여 서비스 워커 개발 과정을 간소화합니다. 웹 서버 성능 최적화, 데이터베이스 관리, 사용자 인증 처리는 Workbox의 주요 목적이 아닙니다.

#### 문제 2
**문제**: 다음 중 Workbox에서 제공하는 캐싱 전략이 아닌 것은?

**정답**: ServerFirst

**해설**: Workbox에서 제공하는 캐싱 전략에는 CacheFirst, NetworkFirst, StaleWhileRevalidate, NetworkOnly, CacheOnly가 있습니다. ServerFirst는 존재하지 않는 전략입니다. CacheFirst는 캐시에서 먼저 응답을 찾고 없는 경우에만 네트워크에서 가져오는 전략이고, NetworkFirst는 네트워크에서 먼저 응답을 가져오고 실패할 경우 캐시에서 가져오는 전략입니다. StaleWhileRevalidate는 캐시된 응답을 즉시 반환하면서 동시에 네트워크 요청을 보내 캐시를 업데이트하는 전략입니다. 각 전략은 리소스 유형과 사용 사례에 따라 적절히 선택하여 사용할 수 있습니다.

#### 문제 3
**문제**: Workbox의 프리캐싱 기능에 대한 설명으로 올바른 것은?

**정답**: 서비스 워커 설치 단계에서 지정된 리소스를 미리 캐싱한다

**해설**: Workbox의 프리캐싱 기능은 서비스 워커 설치 단계에서 지정된 리소스를 미리 캐싱하는 것입니다. `workbox-precaching` 모듈을 사용하여 앱 셸 컴포넌트나 중요한 정적 자산을 서비스 워커가 설치될 때 미리 캐시에 저장함으로써, 이후 요청 시 네트워크 연결 없이도 빠르게 제공할 수 있습니다. 프리캐싱은 개발자가 명시적으로 지정한 리소스에 대해서만 수행되며, 사용자가 방문한 모든 페이지를 자동으로 캐싱하거나, 네트워크 요청이 실패할 때만 캐싱하거나, 서버에서 푸시 알림을 받을 때마다 캐싱하는 것은 아닙니다.

#### 문제 4
**문제**: Workbox를 빌드 도구와 통합하는 주된 이유는 무엇인가요?

**정답**: 서비스 워커 생성 과정 자동화

**해설**: Workbox를 빌드 도구와 통합하는 주된 이유는 서비스 워커 생성 과정을 자동화하기 위함입니다. Webpack, Rollup, Gulp 등의 빌드 도구와 Workbox를 통합하면, 프로젝트 빌드 시 자동으로 최적화된 서비스 워커 파일을 생성할 수 있습니다. 이 과정에서 정적 자산의 해시 값을 자동으로 추적하여 프리캐시 매니페스트를 생성하고, 캐싱 전략을 설정하며, 서비스 워커 코드를 최적화할 수 있습니다. 이를 통해 개발자는 수동으로 서비스 워커 코드를 관리할 필요 없이 효율적으로 PWA를 개발할 수 있습니다. 서버 부하 감소, 데이터베이스 연결 최적화, 사용자 인터페이스 개선은 Workbox를 빌드 도구와 통합하는 주된 이유가 아닙니다.

#### 문제 5
**문제**: 다음 중 `workbox-routing` 모듈의 주요 기능은 무엇인가요?

**정답**: 네트워크 요청 가로채기 및 라우팅

**해설**: `workbox-routing` 모듈의 주요 기능은 네트워크 요청을 가로채고 적절한 핸들러로 라우팅하는 것입니다. 이 모듈을 사용하면 URL 패턴, 정규식, 요청 유형 등 다양한 조건에 따라 네트워크 요청을 처리할 수 있습니다. 예를 들어, 이미지 요청은 CacheFirst 전략으로, API 요청은 NetworkFirst 전략으로 처리하도록 라우팅 규칙을 설정할 수 있습니다. 또한 우선순위 설정, 기본 핸들러, 캐치 핸들러 등을 통해 복잡한 라우팅 로직을 구현할 수 있습니다. 캐시 만료 관리, 푸시 알림 처리, 오프라인 데이터베이스 관리는 `workbox-routing` 모듈의 주요 기능이 아닙니다.

#### 문제 6
**문제**: 오프라인 상태에서 실패한 요청을 저장하고 네트워크 연결이 복구되면 자동으로 재시도하는 Workbox 모듈은?

**정답**: workbox-background-sync

**해설**: 오프라인 상태에서 실패한 요청을 저장하고 네트워크 연결이 복구되면 자동으로 재시도하는 Workbox 모듈은 `workbox-background-sync`입니다. 이 모듈은 Background Sync API를 활용하여 오프라인 상태에서 실패한 네트워크 요청(예: 폼 제출, 데이터 업로드)을 큐에 저장하고, 네트워크 연결이 복구되면 자동으로 재시도합니다. 이를 통해 사용자는 오프라인 상태에서도 작업을 계속할 수 있으며, 애플리케이션은 네트워크 연결이 복구되면 자동으로 데이터를 동기화할 수 있습니다. `workbox-offline`, `workbox-retry`, `workbox-queue`는 존재하지 않는 모듈입니다.

#### 문제 7
**문제**: 다음 중 Workbox를 사용할 때의 장점이 아닌 것은? (복수 응답)

**정답**: 서버 측 렌더링 지원, 데이터베이스 자동 백업

**해설**: 
- 서버 측 렌더링 지원: Workbox는 클라이언트 측 서비스 워커 라이브러리로, 서버 측 렌더링을 직접 지원하지 않습니다. 서버 측 렌더링은 React, Vue, Angular 등의 프레임워크나 Next.js, Nuxt.js 등의 메타 프레임워크에서 제공하는 기능입니다.
- 데이터베이스 자동 백업: Workbox는 데이터베이스 자동 백업 기능을 제공하지 않습니다. 데이터베이스 관리는 IndexedDB API나 다른 데이터베이스 라이브러리를 통해 별도로 구현해야 합니다.
- 서비스 워커 코드 작성 간소화: 이는 Workbox의 실제 장점입니다. Workbox는 복잡한 서비스 워커 코드를 추상화하여 개발자가 더 쉽게 PWA 기능을 구현할 수 있게 해줍니다.
- 자동화된 캐시 관리: 이 역시 Workbox의 장점입니다. Workbox는 캐시 만료, 크기 제한, 버전 관리 등 캐시 관리 기능을 자동화합니다.
- 다양한 캐싱 전략 제공: 이 역시 Workbox의 장점입니다. Workbox는 CacheFirst, NetworkFirst, StaleWhileRevalidate 등 다양한 캐싱 전략을 제공하여 리소스 유형에 맞는 최적의 캐싱 방법을 구현할 수 있게 합니다.
